  ...
  <div class="portlet box grey-cascade">
    ...
    <div class="portlet-body">
      <div class="table-toolbar">
        <div class="row">
          <div class="col-md-6">
            <div class="btn-group"> @if (check_permission('create_user'))<a id="sample_editable_1_new" class="btn green" href="{{ url('users/create') }}"> {{ trans('text.add_user') }} <i class="fa fa-plus"></i> </a>@else <a href=""></a> @endif  </div>
          </div>
          <div class="col-md-6">
      ...
      <table class="table table-striped table-bordered table-hover" id="users">
        <thead>
          <tr>
            ...
            <th> {{ SortableTrait::link_to_sorting_action('email', trans('text.email')) }} </th>
            <th> {{ SortableTrait::link_to_sorting_action('created_at', trans('text.register_date')) }} </th>
            @if (check_permission('edit_user'))<th> {{ trans('text.edit') }} </th>@endif
        ...
        <tr class="odd gradeX">
         ...
         @if (check_permission('edit_user')) <td> {{ link_to("users/{$user->user_id}/edit", trans('text.edit')) }} </td> @endif
          <td>{{ ($user->user_status)? '<span class="label label-sm label-success">' . trans('text.active') . '</span>' : '<span class="label label-sm label-danger">' . trans('text.blocked') . '</span>' }}&nbsp; &nbsp; @if(check_permission('disable_user')) {{ link_to("users/{$user->user_id}/update_status", ($user->user_status) ? trans('text.block'):trans('text.activate')) }} @endif</td>
        </tr>
        @endforeach
          </tbody>
