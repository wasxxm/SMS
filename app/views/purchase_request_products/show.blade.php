...
<div class="portlet gren">
  <div class="portlet-title">
    ...
    <div class="actions"> @if(check_permission('reject_purchase_request')) <a href='{{ url("purchase_requests/$purchase_request->purchase_request_id/reject") }}' class="btn red btn-sm"> <i class="fa fa-pencil"></i> {{ trans('text.reject') }} </a> @endif
      @if(check_permission('approve_purchase_request')) <a href='{{ url("purchase_requests/$purchase_request->purchase_request_id/approve") }}' class="btn green btn-sm"> <i class="fa fa-pencil"></i> {{ trans('text.approve') }} </a> @endif
      ...
      @if(check_permission('delete_purchase_request')) <a href='{{ url("purchase_requests/$purchase_request->purchase_request_id/delete") }}' class="btn red btn-sm"> <i class="fa fa-pencil"></i> {{ trans('text.delete') }} </a> @endif </div>
  </div>
    ...
    <table class="table table-striped table-bordered table-hover" id="view-user">
      <tbody>
        ...
        <tr>
          ...
          <td width="330"><strong>{{ format_product_names($purchase_request->purchase_product_ids, $purchase_request->purchase_request_id) }}</strong></td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td width="330"><strong>{{format_date($purchase_request->purchase_request_edit_date)}}</strong></td>
        </tr>
      </tbody>
    </table>
  </div>
